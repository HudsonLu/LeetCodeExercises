# AES-128 from scratch (educational)
...
def key_expansion(key):
    ...
    for i in range(4):
        words.append(key_bytes[4*i:4*(i+1)])
    ...
    for i in range(4, 44):
        ...
        new_word = []
        for j in range(4):
            new_word.append(words[i - 4][j] ^ temp[j])
    ...
    for r in range(11):
        round_key_bytes = []
        for i in range(4):
            round_key_bytes.extend(words[r * 4 + i])
...
# simple padding with spaces to make 16 bytes
text = "Jumbo land"
plaintext = text.ljust(16).encode("utf-8")   # pad with spaces to the left
...
print("Plaintext block    :", plaintext)
print("Ciphertext (hex)   :", ciphertext.hex())
print("Decrypted block    :", decrypted)
print("Decrypted text     :", decrypted.decode("utf-8").rstrip())